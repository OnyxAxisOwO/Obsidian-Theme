...
> 在线预览部署在 Cloudflare Pages：发布整个仓库即可，根目录的 [`_redirects`](_redirects) 会把站点根路径指向 `/preview/`。
...
├── preview/index.html                # 可交互在线/离线预览（多页面路由，复用真实主题资源）
├── _redirects                        # Cloudflare Pages：站点根 → /preview/
├── dist/                             # 预构建 .ipk
